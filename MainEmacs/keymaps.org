...
(start/leader-keys
	"I" '(:ignore t :wk "Inline image")
  "I T" '(iimage-mode :wk "Toggle inline images in buffer")
	"I t" '(my/image-preview-hover-mode :wk "Toggle image preview at point")
	"I p" '((lambda ()
						(interactive)
						(let ((path (my/image-preview--extract)))
							(if path
									(my/image-preview--show path (current-buffer))
								(message "No image path on this line → [%s]"
												 (buffer-substring-no-properties
													(line-beginning-position) (line-end-position))))))
					:wk "Preview image at point")
  "I P" '(my/image-preview-close :wk "Close image preview")
	)
