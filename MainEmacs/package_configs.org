...
(use-package doom-modeline
  ...
  ;; Diagnostics (Flycheck/Flymake) — 'auto picks whichever checker is active
...
(defvar my/lsp-mode-list
  '(bash-mode
    ...
    csharp-mode
    cmake-mode
    css-mode css-ts-mode scss-mode lsp-tailwindcss-mode  ;; tailwind minor-mode variant listed
    dockerfile-mode
    eslint-mode
    gdscript-mode
    go-mode
    ...
    java-mode
    js-mode javascript-ts-mode jsx-ts-mode typescript-mode typescript-ts-mode tsx-ts-mode
    json-mode yaml-mode
    lua-mode
    markdown-mode
    prisma-mode
...
(use-package lsp-mode
  ...
  (setq lsp-keymap-prefix "C-c l"
        ...
        lsp-diagnostics-provider :flycheck  ;; replaces deprecated lsp-prefer-flymake nil
	...
	;; This stops LSP from trying to watch your entire home directory or system folders
	(setq lsp-file-watch-ignored-directories 
				'("[/\\\\]\\.git\\'" 
					...
					"[/\\\\]\\.cache\\'"
					"[/\\\\]vendor\\'"
					"[/\\\\]/run/sudo\\'")) ; Fixes that permission denied error in your logs
	
	(setq lsp-html-workspace-max-files 500)
  ...
  ;; replace XXX-mode with concrete major-mode[e. g. python-mode]
	:hook ((lsp-mode . (lambda ()
                       ...
                       ;; SKIP html/css/scss: lsp-format-buffer runs AFTER auto-rename-tag edits
                       ;; the closing tag, then LSP reformats from its own state and reverts the change.
                       ;; Skipping these modes fixes the auto-rename-tag revert-on-save bug.
                       (unless (derived-mode-p 'html-mode 'html-ts-mode 'mhtml-mode
         ...
         (css-ts-mode . lsp-deferred) ;; to install: M-x lsp-install-server RET css-ls
         ;; Debugger: No
         ...
         (html-ts-mode . lsp-deferred) ;; to install: M-x lsp-install-server RET html-ls
         ;; Debugger: No
         ...
         (js-ts-mode . lsp-deferred) ;; to install: M-x lsp-install-server RET ts-ls or curl -fsSL https://deno.land/install.sh | sh
         (jsx-mode . lsp-deferred) ;; to install: M-x lsp-install-server RET ts-ls or curl -fsSL https://deno.land/install.sh | sh
         ...
         (typescript-ts-mode . lsp-deferred) ;; to install: M-x lsp-install-server RET ts-ls or curl -fsSL https://deno.land/install.sh | sh
         ;; Debugger: Yes (browser devtools / node debug)
...
*** Image Preview at Point (done)
#+begin_src emacs-lisp
;; ============================================================
;; Inline Image Preview — native child frame, no extra package
;; SPC I p  → peek image on current line
;; SPC I P  → close preview
;; SPC I T  → toggle auto-hover mode
;; ============================================================

;; enabled initially add this hook at the end

;; ;; ── Global Activation Hooks (All Code & Text Files) ──────────
;; (add-hook 'prog-mode-hook #'my/image-preview-hover-mode)
;; (add-hook 'text-mode-hook #'my/image-preview-hover-mode)

;; disabled by default initialy

(require 'cl-lib)

(defcustom my/image-preview-size 350
  "Max width/height in pixels." :type 'integer :group 'convenience)
(defcustom my/image-preview-delay 0.5
  "Idle seconds before hover fires." :type 'number :group 'convenience)
(defcustom my/image-preview-video-seek "00:00:03"
  "Timestamp to grab from a video for the thumbnail (HH:MM:SS or N%)."
  :type 'string :group 'convenience)

(defvar my/image-preview--frame      nil)
(defvar my/image-preview--timer      nil)
(defvar my/image-preview--displayed  nil "Absolute path currently shown.")
(defvar my/image-preview--source-buf nil "Buffer that opened the preview.")
(defvar my/image-preview--thumb-cache (make-hash-table :test #'equal)
  "Cache: video abs-path -> temp thumbnail JPEG path.")

;; ── Dedicated preview buffer ──────────────────────────────────
(defun my/image-preview--buf ()
  (when-let ((old (get-buffer " *img-preview*")))
    (when (and my/image-preview--frame
               (frame-live-p my/image-preview--frame))
      (set-window-dedicated-p (frame-root-window my/image-preview--frame) nil)
      (set-window-buffer (frame-root-window my/image-preview--frame)
                         (get-buffer-create " *img-preview-placeholder*")))
    (kill-buffer old))
  (let ((b (get-buffer-create " *img-preview*")))
...
;; ── Extract image OR video path under cursor universally ─────
(defun my/image-preview--extract ()
  (let ((path (thing-at-point 'filename t)))
    (when (and path
               (string-match-p "\\.\\(png\\|jpe?g\\|gif\\|svg\\|webp\\|bmp\\|ico\\|mp4\\|mkv\\|webm\\|mov\\|avi\\|flv\\|wmv\\|m4v\\|ogv\\)$" (downcase path)))
      ;; Corrected regex: Safely strips surrounding quotes, brackets, or parens
      (replace-regexp-in-string "^[\"'<[(]+\\|[]\"'>)]+$" "" path))))

;; ── Is this path a video file? ────────────────────────────────
(defun my/image-preview--video-p (path)
  (string-match-p
   "\\.\\(mp4\\|mkv\\|webm\\|mov\\|avi\\|flv\\|wmv\\|m4v\\|ogv\\)$"
   (downcase path)))

;; ── Resolve path relative to buffer file (Smarter Web Paths) ─
(defun my/image-preview--resolve (path)
  (let* ((default-dir (or (and (buffer-file-name)
                               (file-name-directory (buffer-file-name)))
                          default-directory))
         ;; Standard resolution
         (standard-path (if (file-name-absolute-p path) 
                            path 
                          (expand-file-name path default-dir)))
         ;; Web resolution: strip leading slash and search relative to current file
         (stripped-path (replace-regexp-in-string "^/" "" path))
         (web-path (expand-file-name stripped-path default-dir)))
    
    ;; If the literal path exists, use it. Otherwise, assume it's a web path.
    (if (file-exists-p standard-path)
        standard-path
      web-path)))

;; ── Extract a thumbnail from a video ─────────────────────────
(defun my/image-preview--video-thumbnail (video-abs)
  (or (gethash video-abs my/image-preview--thumb-cache)
      (let* ((thumb (make-temp-file "emacs-vid-thumb-" nil ".jpg"))
             (exit  (call-process "ffmpegthumbnailer" nil nil nil
                                  "-i" video-abs
                                  "-o" thumb
                                  "-s" (number-to-string (* my/image-preview-size 2))
                                  "-t" my/image-preview-video-seek
                                  "-c" "jpeg" "-q" "3" "-f")))
        (if (and (= exit 0)
                 (file-exists-p thumb)
                 (> (file-attribute-size (file-attributes thumb)) 0))
            (progn (puthash video-abs thumb my/image-preview--thumb-cache) thumb)
          (let ((exit2 (call-process "ffmpegthumbnailer" nil nil nil
                                     "-i" video-abs "-o" thumb
                                     "-s" (number-to-string (* my/image-preview-size 2))
                                     "-t" "0%" "-c" "jpeg" "-q" "3" "-f")))
            (when (and (= exit2 0)
                       (file-exists-p thumb)
                       (> (file-attribute-size (file-attributes thumb)) 0))
              (puthash video-abs thumb my/image-preview--thumb-cache)
              thumb))))))

;; ── Get or create the child frame ────────────────────────────
(defun my/image-preview--get-frame ()
  (when (and my/image-preview--frame
             (not (frame-live-p my/image-preview--frame)))
    (setq my/image-preview--frame nil))
  (or my/image-preview--frame
      (let* ((bg (face-background 'default nil t))
             ...
             (f  (make-frame
                  `((parent-frame           . ,(selected-frame))
                    ...
                    (width  . 10)
                    (height . 4)))))
        (setq my/image-preview--frame f)
        f)))

;; ── Show (Modified for cl-defun and bottom-right positioning) 
(cl-defun my/image-preview--show (path source-buf)
  (unless path (cl-return-from my/image-preview--show))
  (let ((source-win (get-buffer-window source-buf)))
    (unless (window-live-p source-win)
      (cl-return-from my/image-preview--show))
    (condition-case err
        (let* ((abs      (my/image-preview--resolve path))
               (img-path (if (my/image-preview--video-p abs)
                             (my/image-preview--video-thumbnail abs)
                           abs)))
          (unless (and img-path (file-exists-p img-path))
            (cl-return-from my/image-preview--show))
          
          (clear-image-cache img-path)
          
          (let* ((img-type (image-type-from-file-name img-path))
                 (max-image-size nil)
                 (img      (let ((inhibit-message t)
                                 (message-log-max nil))
                             (create-image img-path img-type nil
                                           :max-width  my/image-preview-size
                                           :max-height my/image-preview-size)))
                 (size      (or (ignore-errors (image-size img t))
                                (cons my/image-preview-size my/image-preview-size)))
                 (iw        (car size))
                 (ih        (cdr size))
                 (buf       (my/image-preview--buf))
                 (frm       (my/image-preview--get-frame))
                 (parent    (window-frame source-win))
                 (border    (* 2 (frame-parameter frm 'internal-border-width)))
                 (char-w    (frame-char-width  frm))
                 (char-h    (frame-char-height frm))
                 (cols      (ceiling (/ (float (+ iw border)) char-w)))
                 (rows      (ceiling (/ (float (+ ih border)) char-h)))
                 (frm-pixel-w (* cols char-w))
                 (frm-pixel-h (* rows char-h))
                 (pos-x (- (frame-pixel-width parent) frm-pixel-w 15))
                 (pos-y (- (frame-pixel-height parent) frm-pixel-h 15)))
            ...
            (set-frame-parameter frm 'width  cols)
            (set-frame-parameter frm 'height rows)
            (set-frame-parameter frm 'left   pos-x)
            (set-frame-parameter frm 'top    pos-y)
            (set-window-dedicated-p (frame-root-window frm) nil)
            (set-window-buffer      (frame-root-window frm) buf)
            (set-window-dedicated-p (frame-root-window frm) t)
            (make-frame-visible frm)
            (redraw-frame frm)
            (raise-frame frm)
            (setq my/image-preview--displayed  abs
                  my/image-preview--source-buf source-buf)))
      (error
       (message "my/image-preview: suppressed error: %S" err)
       (my/image-preview-close)))))

;; ── Hide ─────────────────────────────────────────────────────
(defun my/image-preview-close ()
  (interactive)
  (when my/image-preview--timer
    (cancel-timer my/image-preview--timer)
    (setq my/image-preview--timer nil))
  (when (and my/image-preview--frame (frame-live-p my/image-preview--frame))
    (make-frame-invisible my/image-preview--frame))
  (setq my/image-preview--displayed  nil
        my/image-preview--source-buf nil))

;; ── Close when switching away from the source buffer ─────────
(defun my/image-preview--on-buffer-change ()
  (when (and my/image-preview--source-buf
             (not (eq (current-buffer) my/image-preview--source-buf)))
    (my/image-preview-close)))

(add-hook 'buffer-list-update-hook #'my/image-preview--on-buffer-change)

;; ── Hover hook ───────────────────────────────────────────────
(defun my/image-preview--post-command ()
  (let* ((path (my/image-preview--extract))
         (abs  (when path (my/image-preview--resolve path)))
         (buf  (current-buffer)))
    (cond
     ((and abs (not (equal abs my/image-preview--displayed)))
      (when my/image-preview--timer
        (cancel-timer my/image-preview--timer))
      (setq my/image-preview--timer
            (run-with-idle-timer my/image-preview-delay nil
                                 #'my/image-preview--show path buf)))
     ((not path)
      (my/image-preview-close)))))

;; ── Minor mode ───────────────────────────────────────────────
(define-minor-mode my/image-preview-hover-mode
  "Auto-preview image/video paths under cursor."
  :lighter " [Img]"
  (if my/image-preview-hover-mode
      (add-hook  'post-command-hook #'my/image-preview--post-command nil t)
    (remove-hook 'post-command-hook #'my/image-preview--post-command t)
    (my/image-preview-close)))
...
;; Prevent electric-pair from auto-inserting '>' after '<' in HTML modes.
;; The tag-closing logic (sgml-quick-keys / my/insert-slash-and-maybe-close-tag)
;; already handles '>' insertion, so we don't want double '>' characters.
(defun my/html-inhibit-angle-pair (c)
  "Inhibit electric pairing of < > in HTML-like buffers."
  (if (char-equal c ?<)
      t
...
(use-package dap-mode
  ...
  (require 'dap-python)      ;; Python (debugpy)
	(when (locate-library "dap-java")
		(require 'dap-java))        ;; Java (requires lsp-java)
...
(use-package colorful-mode
  :ensure t
  ;; html-mode derives from text-mode not prog-mode so we need both hooks
  :hook ((prog-mode . colorful-mode)
         (html-mode . colorful-mode)
         (html-ts-mode . colorful-mode)
  ...
  (setq colorful-extra-color-keyword-functions
        '(colorful-add-hex-colors
          ((css-mode css-ts-mode scss-mode html-mode html-ts-mode web-mode js-mode js2-mode) . 
           (colorful-add-css-variables-colors
            colorful-add-rgb-colors
            colorful-add-hsl-colors
            colorful-add-oklab-oklch-colors
...
(use-package flycheck
  ...
  :config
  ;; org-lint flycheck checker has a bug where it returns a marker object
  ;; instead of a line number, causing "Wrong type argument: number-or-marker-p"
  ;; errors. Disable it since it's not useful anyway.
...
(use-package copilot
  ...
  :bind (:map copilot-completion-map
              ...
              ("M-e"               . copilot-clear-overlay))
  :config
  ;; Suppress -32800 "Request was cancelled" — these go through copilot--log
  (advice-add 'copilot--log :around
              (lambda (orig-fn level msg &rest args)
                (unless (and (string-match-p "-32800\\|Request was cancelled"
                                            (apply #'format msg args)))
                  (apply orig-fn level msg args))))

  ;; Fix company integration — strip preview frontends so they don't
  ;; conflict with Copilot's inline overlay
  (with-eval-after-load 'company
    (setq company-frontends
          (cl-remove-if (lambda (f)
                          (memq f '(company-preview-frontend
...
(use-package beacon
  ...
  (beacon-blink-when-buffer-changes t) ;; Blink when the buffer changes
	(beacon-dont-blink-commands nil) ;; dont allow evil mode to overwrite beacon blink
  :config
...
;;                                 (list
;;                                  ;; LSP/Eglot CAPFs first (language-aware completions)
;;                                  ;; Using fboundp + #' instead of bound-and-true-p to get
;;                                  ;; a proper function reference, not a variable value.
;;                                  ;; delq nil removes any that aren't loaded yet.
...
(use-package cape
  :ensure t
  :config
  ;; Prevent LSP jsonrpc errors from killing completion when LSP drops connection
  (advice-add 'lsp-completion-at-point :around #'cape-wrap-silent)

  ;; For prog-mode languages (JS, TS, C, Python, etc.)
  (add-hook 'prog-mode-hook
            (lambda ()
              (setq-local completion-at-point-functions
                          (delq nil
  ...
  ;; CSS/SCSS derive from text-mode NOT prog-mode so prog-mode-hook never fires.
  ;; We need a separate hook to set up capf properly for CSS.
...
(defun do-yas-expand ()
  ;; updated from deprecated yas/ API (yasnippet v0.7) to current yas- API
...
(use-package company-web
  :ensure t
	:hook ((css-ts-mode html-ts-mode web-mode html-mode css-mode) . (lambda ()
																																		(add-to-list 'company-backends 'company-web)))
	)

;; CSS completions fallback when LSP crashes (e.g. with many block comments).
;; company-css is built-in and never depends on LSP/jsonrpc.
;; (dolist (hook '(css-mode-hook
;;                 css-ts-mode-hook
;;                 scss-mode-hook))
;;   (add-hook hook
...
;;                           '((company-capf :with company-css company-yasnippet)
;;                             company-css
...
;; Safe company-capf wrapper — catches jsonrpc crashes silently so
;; company-css can take over without spamming error messages.
(defun my/safe-company-capf (command &rest args)
  (condition-case nil
...
;;   (condition-case err
;;       (apply #'company-capf command args)
;;     (jsonrpc-error nil)   ; catch JSON-RPC "No current connection" errors
;;     (error nil)))
...
(dolist (hook '(css-mode-hook
  ...
  (add-hook hook
            (lambda ()
              ...
              (setq-local company-backends
                          '((my/safe-company-capf  ;; LSP when alive = gives docs
                             :with company-css)    ;; css built-in always as fallback
                            company-css
                            company-dabbrev-code
                            company-dabbrev
                            company-files)))))
...
(dolist (hook '(c-mode-hook
                ...
                c-ts-mode-hook
                c++-ts-mode-hook
                objc-mode-hook)) ;; Added objc just in case
  (add-hook hook
            (lambda ()
              ;; 1. Stop LSP from overriding our backends
              (setq-local lsp-completion-provider :none) 
              
              ;; 2. Set our safe backends
              (setq-local company-backends
                          '((my/safe-company-capf
...
(defun my/setup-safe-lsp-completion ()
  "Ensure LSP completion is wrapped in our safe handler."
  (setq-local lsp-completion-provider :none)
  (setq-local company-backends 
              '((my/safe-company-capf :with company-yasnippet)
                company-dabbrev-code 
                company-files 
                company-dabbrev)))


;; Apply to everything that isn't already specifically handled
(add-hook 'prog-mode-hook #'my/setup-safe-lsp-completion)



;; ;; Fix: company-preview-frontend (inline ghost text) renders on the wrong line
;; ;; in CSS files with block comments above point. The comments confuse its
;; ;; position tracking. The tooltip popup (company-box) still works fine.
;; (dolist (hook '(css-mode-hook
;;                 css-ts-mode-hook
;;                 scss-mode-hook))
;;   (add-hook hook
...
;;                             company-box-frontend)))))  ;; drop company-preview-frontend
...
(defun my/company-sort-directories-first (candidates)
  "Sort directories before files in Company completions."
  (let ((is-file-completion
         (or (eq company-backend 'company-files)
             (and (listp company-backend) (memq 'company-files company-backend))
             ;; Handle cape-file and LSP file completions
             (when candidates
               (let ((kind (company-call-backend 'kind (car candidates))))
                 (memq kind '(file folder)))))))
    (if is-file-completion
        (let (dirs others)
          (dolist (cand candidates)
            ;; In Emacs file completions, directories always end with a slash "/"
            (if (and (stringp cand) (string-suffix-p "/" cand))
                (push cand dirs)
              (push cand others)))
          ;; Append non-directories after directories, keeping existing internal sort
          (append (nreverse dirs) (nreverse others)))
      ;; If not a file completion, leave candidates untouched
      candidates)))
...
(use-package company
  ...
  (company-selection-wrap-around t)
	(company-transformers '(company-sort-by-occurrence my/company-sort-directories-first)) ;;
  ...
  (global-company-mode t)
	...
	No preview frontends — Copilot owns the inline overlay
...
(use-package company-box
  ...
  ;; ── 3. Valid CSS Named Colors ──────────────────────────────────────────────
  (defvar my/css-named-colors
    '(("aliceblue" . "#f0f8ff") ("antiquewhite" . "#faebd7") ("aqua" . "#00ffff")
      ...
      ("yellowgreen" . "#9acd32"))
    "Full CSS named colors alist.")

  ;; ── 4. HEX PALETTE ────────────────────────────────────────────────────────
  (defun my/get-emacs-hexes ()
    (let ((hexes nil))
      (dolist (c (defined-colors))
        (let ((v (color-values c)))
          (when v
            (push (format "#%02x%02x%02x"
                          (ash (nth 0 v) -8)
                          (ash (nth 1 v) -8)
                          (ash (nth 2 v) -8))
                  hexes))))
  ...
  (defvar my/basic-hex-palette
    '("#f8f9fa" "#f1f3f5" "#e9ecef" "#dee2e6" "#ced4da" "#adb5bd" "#868e96" "#495057" "#343a40" "#212529"
      "#fff0f2" "#ffe4e6" "#fecdd3" "#fda4af" "#fb7185" "#f43f5e" "#e11d48" "#be123c" "#9f1239" "#881337"
  ...
  (defvar my/master-hex-palette
    (delete-dups (append my/basic-hex-palette (my/get-emacs-hexes))))

  ;; ── 5. DEDICATED HEX BACKEND ──────────────────────────────────────────────
  ;; Uses post-self-insert-hook to grab control the instant '#' is typed,
  ;; bypassing clangd/LSP entirely. This is the only reliable approach when
  ;; clangd treats '#' as a preprocessor directive start.

  (defun my/hex-color-prefix-at-point ()
    "Return #hex prefix at point or nil. Scans backwards past hex digits and #."
    (let ((end (point))
          (pos (point)))
      (while (and (> pos (point-min))
                  (let ((ch (char-before pos)))
                    (or (and (>= ch ?0) (<= ch ?9))
                        ...
                        (= ch ?#))))
        (setq pos (1- pos)))
      (let ((s (buffer-substring-no-properties pos end)))
				(when (string-match-p "\\`#[0-9a-fA-F]*\\'" s) s))))


  (defun company-hex-color (command &optional arg &rest _)
    "Company backend for hex color codes. Bypasses LSP/clangd."
    (interactive (list 'interactive))
    (cl-case command
      (interactive  (company-begin-backend 'company-hex-color))
      (prefix       (my/hex-color-prefix-at-point))
      ;; (candidates
      ;;  (let ((pfx (downcase arg)))
      ;;    (cl-remove-if-not (lambda (h) (string-prefix-p pfx h))
      ;;                      my/master-hex-palette)))
			(candidates
			 (let* ((pfx (downcase arg))
							(matches (cl-remove-if-not (lambda (h) (string-prefix-p pfx h))
																				 my/master-hex-palette)))
				 (or matches nil)))
      (kind         'color)
      (annotation   " hex")
  ...
  ;; Hook that fires after EVERY keystroke. When we detect we're inside a
  ;; #hex context, we immediately invoke our backend, which prevents clangd
  ;; from ever getting a turn for this completion session.
(defun my/maybe-start-hex-completion ()
    "Start hex color completion immediately when typing inside a #hex literal."
    (when (and (not (minibufferp))
               (not company-candidates) ; don't interrupt an active session
               (my/hex-color-prefix-at-point))
      (company-begin-backend 'company-hex-color)))

  (add-hook 'post-self-insert-hook #'my/maybe-start-hex-completion)

  ;; ── 6. Color Swatch Icon ──────────────────────────────────────────────────
  (defun my/company-box-color-swatch (candidate)
    (ignore-errors
      (let* ((cand-str (substring-no-properties candidate))
             (color (cond
                     ((string-match-p "\\`#[0-9a-fA-F]\\{3,8\\}\\'" cand-str) cand-str)
                     ((cdr (assoc (downcase cand-str) my/css-named-colors))))))
        (when color
          (let ((color-sym (intern (concat "my-color-swatch-" color))))
            (unless (assq color-sym company-box-icons-all-the-icons)
              (push (cons color-sym
  ...
  (defun my/company-box--ensure-swatch-first ()
    (unless (eq (car company-box-icons-functions) #'my/company-box-color-swatch)
      (setq company-box-icons-functions
            (cons #'my/company-box-color-swatch
                  (remq #'my/company-box-color-swatch company-box-icons-functions)))))

  (add-hook 'company-box-mode-hook #'my/company-box--ensure-swatch-first))
...
(use-package emms
  ...
  (setq emms-info-asynchronously nil)
	...
	;; Keep playing-time tracking active (needed for custom emms-state modeline segment)
	;; but disable the built-in left-side display -- our custom segment on the right handles it
	(emms-playing-time 1)
	(setq emms-playing-time-display-p nil)  ;; disable left-side built-in time display
  (setq emms-playing-time-style 'time)

	;; Disable the built-in EMMS mode-line title (correct function is emms-mode-line-mode)
        ...
        '(("t" "TODOS / IDEAS / ISUES")
          ...
          ("nP" "Personal Note With custom link" entry (file+headline "~/git/Emacs-Todos/NOTES.org" "PERSONAL")
					...
					("cgp" "Project" entry (file+headline "~/git/Emacs-Todos/emails/gamedev-mails.org" "Projects")
					 "* %^{Project name}\n:PROPERTIES:\n:calendar-id:\tpappanos.gamedev.004@gmail.com\n:END:\nStart: %^{Start date}t\nEnd: %^{Due date}t\n:org-gcal:\n%^{Start date}T--%^{Due date}T\n%^{Description}\n:END:\n\n" :jump-to-captured t)
					("cgd" "Deadline" entry (file+headline "~/git/Emacs-Todos/emails/gamedev-mails.org" "Deadlines")
					 "* %^{Deadline name}'s Deadline\n:PROPERTIES:\n:calendar-id:\tpappanos.gamedev.004@gmail.com\n:END:\n%^{Deadline date and time}T\n:org-gcal:\n%^{Deadline date and time}T\n%^{Description}\n:END:\n\n" :jump-to-captured t)
