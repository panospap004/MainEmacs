...
(use-package company-box
  ...
  ;; ── 3. Valid CSS Named Colors (The official 148 web-safe colors) ───────────
  ...
  ;; ── 4. THE MASSIVE HEX PALETTE GENERATOR ───────────────────────────────────
  
  (defun my/get-emacs-hexes ()
    "Extract only the pure hex codes from Emacs's internal color engine."
    (let ((hexes nil))
      (dolist (c (defined-colors))
        (let ((v (color-values c)))
          (when v
            ;; Convert 16-bit RGB to standard 8-bit CSS Hex
            (push (format "#%02x%02x%02x" 
                          (ash (nth 0 v) -8) 
                          (ash (nth 1 v) -8) 
                          (ash (nth 2 v) -8)) 
                  hexes))))
      (delete-dups hexes))) ; Remove duplicates so we don't get 5 identical blacks

  (defvar my/basic-hex-palette
    '(;; Modern UI Colors (You can still add custom ones here!)
    ...
    "A hand-picked palette of modern UI hex colors.")

  (defvar my/master-hex-palette
    (delete-dups (append my/basic-hex-palette (my/get-emacs-hexes)))
    "The ultimate hex palette: Hand-picked UI colors + 500+ internal Emacs hex codes.")

  ;; ── 5. SMART HEX INJECTION TRANSFORMER ─────────────────────────────────────
  (defun my/company-inject-palette-transformer (candidates)
    "Inject our massive hex palette into the list ONLY when typing a hex code."
    (if (and (stringp company-prefix)
             (string-prefix-p "#" company-prefix))
        (let ((existing-strs (mapcar #'substring-no-properties candidates))
              (injected nil))
          (dolist (hex my/master-hex-palette)
            (when (and (string-prefix-p (downcase company-prefix) hex)
                       (not (member hex existing-strs)))
              (push (propertize hex 'company-kind 'color) injected)))
          (append candidates (nreverse injected)))
      candidates))

  (add-to-list 'company-transformers #'my/company-inject-palette-transformer)

  ;; ── 6. The Dynamic Color Swatch Generator ──────────────────────────────────
  (defun my/company-box-color-swatch (candidate)
    "Identify color candidates, dynamically generate an icon, and return its symbol."
    (ignore-errors
  ...
  ;; Ensure the color swatch runs BEFORE default LSP icon assignments
