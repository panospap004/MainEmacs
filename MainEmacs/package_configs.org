...
;; ;; ── Dedicated preview buffer ──────────────────────────────────
;; (defun my/image-preview--buf ()
;;   ;; Kill the old buffer every time — this is the critical fix.
;;   ;; Reusing the same buffer lets Emacs' glyph cache (keyed by image
;;   ;; TYPE, not path) silently return a stale/blank render for any
;;   ;; image whose type was already shown (e.g. jpeg after jpeg).
;;   ;; A fresh buffer has no glyph history, so every image is
;;   ;; rendered from scratch regardless of type.
...
;; ;;     (message "Image preview → %s [exists: %s]" abs (file-exists-p abs))
;; ;;     (unless (file-exists-p abs) (cl-return-from my/image-preview--show))
;; ;;     ;; Flush the entire image cache — the only reliable way to force
;; ;;     ;; Emacs to reload a jpg/jpeg (type-keyed caching ignores per-file flush)
;; ;;     (clear-image-cache t)
;; ;;     (let* ((img    (create-image abs nil nil
...
;; ;;            (pos    (with-selected-window
;; ;;                        (get-buffer-window source-buf)
...
;; ;;       (set-frame-parameter frm 'top    (+ pos-y line-h))
...
;;     (unless (file-exists-p abs) (cl-return-from my/image-preview--show))
;;     (clear-image-cache t)
;;     (let* ((img-type (image-type-from-file-name abs))   ;; ← explicit type
;;            (img    (create-image abs img-type nil        ;; ← pass it here
;;                                  :max-width  my/image-preview-size
;;                                  :max-height my/image-preview-size))
;;            (size   (image-size img t))
;;            (iw     (car size))
;;            (ih     (cdr size))
;;            (buf    (my/image-preview--buf))               ;; ← now always fresh
;;            (frm    (my/image-preview--get-frame))
;;            (border (* 2 (frame-parameter frm 'internal-border-width)))
;;            (char-w (frame-char-width  frm))
;;            (char-h (frame-char-height frm))
...
;;   "Auto-preview image paths under cursor."
;;   :lighter " [Img]"
...
;; ── Dedicated preview buffer ──────────────────────────────────
;; We kill-and-recreate the buffer on every preview to bust Emacs'
;; type-keyed glyph cache (stale/blank renders when showing the same
;; image type back-to-back, e.g. jpeg after jpeg).
;; Before killing the old buffer we redirect the child frame's window
;; to a throwaway placeholder buffer so the frame's window state stays
;; valid and the window-valid-p crash is avoided on the next redisplay.
(defun my/image-preview--buf ()
  (when-let ((old (get-buffer " *img-preview*")))
    ;; Detach child-frame window BEFORE killing, or Emacs corrupts its
    ;; window state and raises window-valid-p on the next preview.
    (when (and my/image-preview--frame
...
;; Calls ffmpegthumbnailer once per video and caches the result.
  ...
  (or (gethash video-abs my/image-preview--thumb-cache)
      (let* ((thumb (make-temp-file "emacs-vid-thumb-" nil ".jpg"))
        ...
        (if (and (= exit 0)
          ...
          ;; Fallback: seek to 0% for very short videos
    ...
    (condition-case err
        (let* ((abs      (my/image-preview--resolve path))
          ...
          (unless (and img-path (file-exists-p img-path))
            (cl-return-from my/image-preview--show))
          ;; Grab position BEFORE make-frame triggers redisplay
          (let* ((pos    (with-selected-window source-win
                           (window-absolute-pixel-position)))
                 (pos-x  (or (car pos) 0))
                 (pos-y  (or (cdr pos) 0))
                 (line-h (frame-char-height (window-frame source-win))))
            (let* ((img-type (image-type-from-file-name img-path))
                   (img      (create-image img-path img-type nil
                                           :max-width  my/image-preview-size
                                           :max-height my/image-preview-size))
                   (size     (image-size img t))
                   (iw       (car size))
                   (ih       (cdr size))
                   (buf      (my/image-preview--buf))   ; always a fresh buffer
                   (frm      (my/image-preview--get-frame))
                   (border   (* 2 (frame-parameter frm 'internal-border-width)))
                   (char-w   (frame-char-width  frm))
                   (char-h   (frame-char-height frm))
...
(defun my/image-preview--post-command ()
  ...
  (let* ((path (my/image-preview--extract))
         ...
         (buf  (current-buffer)))
    (cond
     ;; New path, or returning to one after closing → schedule show
     ((and abs (not (equal abs my/image-preview--displayed)))
      (setq my/image-preview--timer
            (run-with-idle-timer my/image-preview-delay nil
                                 #'my/image-preview--show path buf)))
     ;; Not on any recognised path → close immediately
     ((not path)
      (my/image-preview-close)))))
