...
;;                   cursor-type        nil
;;                   truncate-lines      t
;;                   ;; [FIX 1] Bypass Emacs safety limits for large images
;;                   max-image-size      nil))
;;     b))

;; ;; ── Extract image OR video path from the current line ────────
;; (defun my/image-preview--extract ()
;;   ;; [FIX 2] Prevent background packages from messing up the regex
;;   (save-match-data
;;     (let ((line (buffer-substring-no-properties
;;                  (line-beginning-position) (line-end-position))))
;;       (catch 'hit
;;         (dolist (pat
;;                  '(;; src="..." href="..." data-src="..." — images
;;                    "\\(?:src\\|href\\|data-src\\)=[\"']\\([^\"'\n]+\\.\\(?:png\\|jpe?g\\|gif\\|svg\\|webp\\|bmp\\|ico\\)\\)[\"']"
;;                    ;; src="..." href="..." data-src="..." — videos
;;                    "\\(?:src\\|href\\|data-src\\)=[\"']\\([^\"'\n]+\\.\\(?:mp4\\|mkv\\|webm\\|mov\\|avi\\|flv\\|wmv\\|m4v\\|ogv\\)\\)[\"']"
;;                    ;; url(...) — images
;;                    "url([\"']?\\([^\"'()\n ]+\\.\\(?:png\\|jpe?g\\|gif\\|svg\\|webp\\|bmp\\|ico\\)\\)[\"']?)"
;;                    ;; bare quoted path — images
;;                    "[\"']\\([^\"'\n ]+\\.\\(?:png\\|jpe?g\\|gif\\|svg\\|webp\\|bmp\\|ico\\)\\)[\"']"
;;                    ;; bare quoted path — videos
;;                    "[\"']\\([^\"'\n ]+\\.\\(?:mp4\\|mkv\\|webm\\|mov\\|avi\\|flv\\|wmv\\|m4v\\|ogv\\)\\)[\"']"))
;;           (when (string-match pat line)
;;             (throw 'hit (match-string 1 line))))))))
...
;; ;; ── Resolve path relative to buffer file ─────────────────────
...
;; ;; ── Show (Modified for Bottom-Right Positioning) ─────────────
;; (defun my/image-preview--show (path source-buf)
...
;;           ;; [FIX 3] Flush the Emacs image cache so bad JPG data doesn't persist
;;           (clear-image-cache img-path)
          
;;           (let* ((img-type (image-type-from-file-name img-path))
;;                  ;; [FIX 4] Silence JPG warnings and bypass size limits to prevent crashes
;;                  (max-image-size nil)
...
;;                  ;; [FIX 5] Provide a fallback if Emacs still can't calculate dimensions
...
;;   ;; Safely kills the frame instantly if you switch buffers.
...
;;   ;; [FIX 6] Keep the smarter timer logic so wiggling mouse doesn't delay load
...
;; (dolist (h '(css-mode-hook css-ts-mode-hook scss-mode-hook
;;              html-mode-hook html-ts-mode-hook mhtml-mode-hook
;;              web-mode-hook js-mode-hook js-ts-mode-hook
;;              typescript-mode-hook typescript-ts-mode-hook tsx-ts-mode-hook))
;;   (add-hook h #'my/image-preview-hover-mode))
